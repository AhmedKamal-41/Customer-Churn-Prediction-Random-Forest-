...
function BackendStatus({ online, onRetry, demoModeEnabled }) {
  if (online === null) return <Badge status="neutral">Checking…</Badge>
  return (
    <div className="flex items-center gap-2">
      <button
        type="button"
        onClick={() => !online && onRetry?.()}
        className="focus:outline-none focus-visible:ring-2 focus-visible:ring-accent rounded"
        aria-label={online ? 'Backend online' : 'Backend offline — click to retry'}
        title={online ? undefined : 'Click to retry'}
      >
        <Badge status={online ? 'success' : 'error'}>Backend: {online ? 'Online' : 'Offline'}</Badge>
      </button>
      {demoModeEnabled && (
        <Badge status="neutral" className="bg-amber-500/20 text-amber-400 border-amber-500/30">Demo Mode</Badge>
      )}
    </div>
  )
}
...
function Sidebar({ onNavigate, recentSessions, activeSessionId, onSelectSession, onNewSession, onDeleteSession }) {
  return (
    <>
      <div className="px-4 py-6 border-b border-border/50">
        <Link to="/" className="font-semibold text-lg text-text no-underline hover:opacity-90" onClick={onNavigate}>
          Churn Assistant
        </Link>
      </div>
      <nav className="p-4 space-y-1 flex-1" aria-label="Main">
        {nav.map(({ path, label }) => (
          <NavLink
            ...
            onClick={onNavigate}
            isActive={(_, location) =>
              path === '/' ? (location.pathname === '/' || location.pathname === '/chat') : location.pathname === path
            }
            className={({ isActive }) =>
          ...
          {recentSessions.slice(0, 5).map((session) => (
              <li key={session.id} className="group flex items-stretch gap-1">
                ...
                >
                  ...
                  <span className="block text-xs text-muted mt-0.5">{formatSessionTime(session.updatedAt)}</span>
                </button>
                {recentSessions.length > 1 && (
                  <button
                    ...
                    className="flex-shrink-0 rounded-xl px-2 py-1.5 text-muted hover:text-red-400 hover:bg-red-500/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent"
                    aria-label={`Remove session ${session.title || 'Untitled'}`}
...
export default function AppShell() {
  ...
  const { online, lastOnlineAt, retry } = useBackendStatus()
  const { demoModeEnabled, enableDemoMode, disableDemoMode } = useDemoMode()
  const { answers, prediction, status, resetChat, rightTab, setChatState, activeSessionId, setActiveSessionId } = useChatApp()
  ...
  return (
    <div className="min-h-screen bg-bg text-text transition-colors duration-200">
      <div className="grid min-h-screen md:grid-cols-[280px_1fr]">
        {/* Sidebar - desktop: in flow; mobile: hidden, shown as drawer */}
        ...
        {/* Main area: header + content (+ optional right panel) */}
        <div className="flex flex-col min-h-0 min-w-0">
          ...
          <header className="sticky top-0 z-30 flex items-center justify-between h-14 px-4 md:px-8 bg-card/95 backdrop-blur border-b border-border shadow-lg shadow-black/20 flex-shrink-0">
            <div className="flex items-center gap-3">
              ...
              <Link to="/" className="font-semibold text-lg text-text hover:opacity-90 no-underline">
                Churn Assistant
              </Link>
            </div>
            <div className="flex items-center gap-3">
              {(location.pathname === '/' || location.pathname === '/chat') && (
                <Button variant="ghost" type="button" onClick={handleNewSession} aria-label="New session">
                  New session
                </Button>
              )}
              <BackendStatus online={online} onRetry={retry} demoModeEnabled={demoModeEnabled} />
              <Button variant="secondary" onClick={resetChat} aria-label="Reset chat">
                Reset chat
              </Button>
              <ThemeToggle />
              <IconButton
